...
FString UDwmGameInstance::GetEconomyPackagePath() const
{
    ...
    const FString ShippedPath = FPaths::ConvertRelativePathToFull(
        FPaths::Combine(FPaths::ProjectContentDir(), TEXT("Databases/world_economy.db")));

#if WITH_EDITOR
    // In the editor, work on the canonical file itself so the DWMStudio round trip
    // keeps operating on the package it exported.
    return ShippedPath;
#else
    // SHIPPED BUILDS OPEN A PRIVATE COPY, because settling a trade WRITES to this
    // database and Content/ is not writable once installed -- the installer targets
    // Program Files, where a standard user has read access only. Opening the shipped
    // file directly is what produced "disk I/O error" on Maria's trade.
    static const FString WritablePath = FPaths::ConvertRelativePathToFull(
        FPaths::Combine(FPlatformProcess::UserSettingsDir(),
            TEXT("DWM_Dev"), TEXT("world_economy.db")));

    IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
    if (!PlatformFile.FileExists(*WritablePath))
    {
        PlatformFile.CreateDirectoryTree(*FPaths::GetPath(WritablePath));
        if (PlatformFile.CopyFile(*WritablePath, *ShippedPath))
        {
            UE_LOG(LogTemp, Log, TEXT("[DWM Economy] Seeded the economy database at '%s'."),
                *WritablePath);
        }
        else
        {
            UE_LOG(LogTemp, Error,
                TEXT("[DWM Economy] Could not copy '%s' to '%s'. Trading will fail. ")
                TEXT("Is Content/Databases staged into the build?"),
                *ShippedPath, *WritablePath);
        }
    }
...
void UDwmGameInstance::SpawnDemoTradeTerminal()
{
    ...
    {
        // TAG FIRST, for the same reason the seats use one: mesh-path matching picks
        // whichever rocking chair is nearest, and "nearest" resolved differently in
        // the cooked build.
        bool bAnchoredByTag = false;
        for (TActorIterator<AActor> TagIt(World); !bAnchoredByTag && TagIt; ++TagIt)
        {
            if (*TagIt && TagIt->ActorHasTag(TEXT("DWM_MariaSeat")))
            {
                AnchorNpc = *TagIt;
                bAnchoredByTag = true;
                UE_LOG(LogTemp, Log,
                    TEXT("[DWM Economy] Terminal anchored to tagged '%s' (DWM_MariaSeat)."),
                    *GetNameSafe(*TagIt));
                break;
            }
        }

        for (TActorIterator<AActor> It(World); !bAnchoredByTag && It; ++It)
        {
            AActor* Candidate = *It;
